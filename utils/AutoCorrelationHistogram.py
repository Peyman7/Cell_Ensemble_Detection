...
def CrossCorrelation_Histogram(Spikes1, Spikes2, Bin_size, t_start, t_end, lag, plot):
    ...
    if Spikes1 == Spikes2:
        ...
        Spikes = neo.SpikeTrain(Spikes1*pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)
        Qmatrix = conv.BinnedSpikeTrain(Spikes, bin_size=Bin_size * pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)
       ...
       # Cross_corr_histogram[0,lag+1:2*lag+1]= Cross_corr_lag[lag+1:2*lag+1]/[xx for xx in np.arange(1,lag+1,1)]
        ...
        Spikes1 = neo.SpikeTrain(Spikes1*pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)
        Spikes2 = neo.SpikeTrain(Spikes2*pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)

        Qmatrix1 = conv.BinnedSpikeTrain(Spikes1, bin_size=Bin_size * pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)
        Qmatrix2 = conv.BinnedSpikeTrain(Spikes2, bin_size=Bin_size * pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)
...
######### MAIN CODE ################
Bin_size = 10  # unit in 0.1 msec
lag = 50
t_ss = 18094635.40  # t_start in 0.1 ms scale
t_es = 254677249.08# t_end in 0.1 ms scale

with open('cell05.txt') as f:
    data = f.read()
data = data.split()

Spikes = [int(data[i])/100 for i in range(len(data))] ## convert spikes times to 0.1 msec
Spikes = [xx for xx in Spikes if xx>=t_ss and xx<=t_es]


Cross_corr_lag, peak_CCH, median_CCH = CrossCorrelation_Histogram(Spikes, Spikes, Bin_size, t_ss, t_es, lag, plot=True) 
...
Spikes = neo.SpikeTrain(Spikes*pq.ms, t_start = t_ss * pq.ms, t_stop = t_es * pq.ms)
...
s1 = np.zeros((1,Qmatrix_arr[0].size))

for s in range(s1.size):
    s1[0,s] = Qmatrix_arr[0,s]
    

Cross_corr_histogram = np.zeros((1,101))
Cross_corr = signal.correlate(s1[0], s1[0], mode='same')
Cross_corr_lag = Cross_corr[np.argmax(Cross_corr)-50:np.argmax(Cross_corr) + 51]
#Cross_corr_lag = Cross_corr[50:100]

Cross_corr_lag[np.argmax(Cross_corr_lag)]=0
#Cross_corr_lag = Cross_corr_lag/20
Cross_corr_histogram[0,0:50]= Cross_corr_lag[0:50]/[xx for xx in np.arange(50,0,-1)]
Cross_corr_histogram[0,51:101]= Cross_corr_lag[51:101]/[xx for xx in np.arange(1,51,1)]

### Plot Autocorrelation Historam
fig, ax = plt.subplots(1,1, figsize=(6,4))
ax.bar(np.arange(-50,51), Cross_corr_lag/100)
#ax[1].bar(np.arange(-50,51), Cross_corr_histogram[0,:])
...
#### Autocorrelation peak and median location
peak_CCH = np.argmax(Cross_corr_lag[51:101])
median_CCH= (Cross_corr_lag[51:101].tolist()).index(np.percentile(Cross_corr_lag[51:101].tolist(),50,interpolation='nearest'))
