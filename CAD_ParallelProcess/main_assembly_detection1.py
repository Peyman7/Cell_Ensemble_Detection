...
def cell_assembly_detection1(binned_spiketrain, max_lag, reference_lag=2,
...
###########################PARALLEL RUN ###################################################################
def process_spike_train(sp_list):
    try:
        return CAD_p(sp_list)
    except Exception as e:
        print(f"Error processing spike train {sp_list}: {e}")
        return None
...
###parameters
#bin_size = [30, 50, 70, 100, 250, 350, 500, 650, 850, 1000]   ## in NMSA unit (0.1 ms)
bin_size = [750,900,1000]
#bin_size = [250,350,500,650,750,900,1000]
max_lags = 10
...
splist = [dict([('spike',spT), ('t_st',t_s) , ('t_en', t_e), ('rat',rat_n), ('date_rec', date), ('lag', max_lags), ('bin', bin_size[pp])]) for pp in range(len(bin_size))]
#size_list = len(split)
...
if __name__ == '__main__':
    ...
    splist = [dict([('spike', spT), ('t_st', t_s), ('t_en', t_e), ('rat', rat_n),
                    ('date_rec', date), ('lag', max_lags), ('bin', b)]) for b in bin_size]
